...
#pragma region webApi
  ...
  switchConfigHandler->onRequest([](AsyncWebServerRequest *request,
                                    JsonVariant &root) {
    AsyncJsonResponse *response = new AsyncJsonResponse();
    JsonObject doc = response->getRoot().to<JsonObject>();
    ...
    */
    ...
    for (int i = 0; i < _MAX_SWITCH_ID_; i++) {
      ...
      // 4
      //  switch is configured but incoming type is different require a startup
      //  process
      if (SwitchObjects[i]->getType() != incomingType) {
        ...
        // 5
        // pin is different
        if (SwitchObjects[i]->getPinNumber() !=
            IncomingSwitch[i]["pin"].as<unsigned int>()) {
          reboot = true;
          continue;
        }
        ...
        if (SwitchObjects[i]->getType() == static_cast<int>(SwTypeServo)) {
          ServoOutput *servo = static_cast<ServoOutput *>(SwitchObjects[i]);
          servo->setMovingTime(
              IncomingSwitch[i]["moveTime"].as<unsigned int>());
        }
      }
    }
