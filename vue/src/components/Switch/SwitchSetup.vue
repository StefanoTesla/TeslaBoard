          ...
          <div class="module_toggle">
            <label class="toggle" for="switch_module_status">
              <input
                ...
                name=""
                type="checkbox"
                id="switch_module_status"
                v-model="switches.enable"
                @change="validate()"
              />
...
const moveSwitch = (id, dir) => {
  if (dir) {
    [switches.value.Switches[id], switches.value.Switches[id - 1]] = [
      switches.value.Switches[id - 1],
      switches.value.Switches[id],
    ];
    [validation.value[id], validation.value[id - 1]] = [
      validation.value[id - 1],
      validation.value[id],
    ];
  } else {
    [switches.value.Switches[id], switches.value.Switches[id + 1]] = [
...
const sanitize = (index) => {
  switches.value.Switches[index].name = switches.value.Switches[index].name
    .replace(/[<>#!?*]/g, "")
    .slice(0, 20);
  switches.value.Switches[index].desc = switches.value.Switches[index].desc
    .replace(/[<>#!?*]/g, "")
    .slice(0, 20);
};
