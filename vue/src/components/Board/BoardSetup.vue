<template>
  ...
  >
    <div class="two_columns">
      <div class="card">
        <div class="setting_table">
          ...
          <div class="setting_row">
            <select
              id="board_locale"
              :class="[localeUnvalid ? 'validation_error' : '']"
              v-model="board.locale"
              @change="validate()"
            >
      ...
      <div class="card">
        <div class="setting_table">
          ...
          <div class="setting_row">
            <input
              id="board_module_name"
              class="identifier"
              type="text"
              v-model="board.identifier"
            />
          </div>
        </div>
      </div>
        ...
        <div>
          ...
          <ul>
            <li v-for="el in inputGPIO">
              <p :class="[el.used === 0 ? 'text-green-400!' : 'text-red-400!']">
                {{ el.pin }}
                <span v-for="module in el.module">{{
                  t(`IOBase.moduleEnum.${module}`)
                }}</span>
              </p>
            </li>
          </ul>
    ...
    <div class="card my-4">
      <div class="sw_header">
        <p>{{ t("board.setting.checkUpdate") }}</p>
      </div>
      <div class="setting_row">
        <button class="green cursor-pointer" @click="checkUpdate()">
          {{ t("board.setting.checkUpdate") }}
        </button>
        <p>{{ t("board.setting.currentVersion") }} {{ swVersion }}</p>
        <template v-if="newVersion.check == true">
          <p v-if="newVersion.version == 0">
            {{ t("board.setting.noUpdate") }}
          </p>
          <p v-else>
            {{ t("board.setting.newUpdate") }} {{ newVersion.version }}
          </p>
        </template>
      </div>
    </div>
    ...
    <div class="card">
      <div class="setting_row">
        <button class="red cursor-pointer" @click="rebootBoard()">
          {{ t("board.setting.reboot") }}
        </button>
      </div>
    </div>
...
watch(
  ...
  (newValue) => {
    ...
    const usedPins = newValue.filter((obj) => obj.pin).map((obj) => obj.pin);
